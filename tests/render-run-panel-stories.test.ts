/**
 * Tests for US-006: Pass stories data into renderRunPanel for badge progress
 *
 ...
 * and uses them for badge progress (not window._lastStories).
 */
...
describe('US-006: renderRunPanel accepts stories as second argument', () => {
  test('AC1: renderRunPanel signature accepts stories default []', () => {
    ...
    assert.ok(result.includes('loop-badge'), 'badge should render with default empty stories');
    assert.ok(result.includes('0%'), 'badge should show 0% with no stories');
  });

  test('AC2: Badge percentage is computed from stories passed to renderRunPanel', () => {
    const stories = [
      { status: 'done' },
      { status: 'done' },
      { status: 'pending' },
    ];
    // window._lastStories is empty — badge must use passed stories
    const result = callRenderRunPanel(makeLoopRun(), stories, []);
    assert.ok(result.includes('2/3'), 'badge should show 2/3 from passed stories');
    assert.ok(result.includes('conic-gradient'), 'badge should use conic-gradient');
  });

  test('AC2: Badge ignores window._lastStories and uses passed stories', () => {
    ...
    assert.ok(result.includes('2/2'), 'badge should use passed stories (2/2), not window (0/3)');
    assert.ok(!result.includes('0/3'), 'badge should NOT show 0/3 (window._lastStories)');
  });
  ...
  test('AC5: When stories is empty, badge shows 0% fill and no crash', () => {
    let threw = false;
    let result = '';
    try {
    ...
    assert.ok(result.includes('0%'), 'badge should show 0%');
  });

  test('AC6: renderRunPanel with stories arg renders correct badge fraction', () => {
    const stories = Array.from({ length: 5 }, (_, i) => ({ status: i < 3 ? 'done' : 'pending' }));
    const result = callRenderRunPanel(makeLoopRun(), stories);
    assert.ok(result.includes('3/5'), 'badge should show 3/5');
  });

  test('AC6: renderRunPanel with all stories done shows 100%', () => {
    const stories = [{ status: 'done' }, { status: 'done' }, { status: 'done' }];
    const result = callRenderRunPanel(makeLoopRun(), stories);
    assert.ok(result.includes('3/3'), 'badge should show 3/3 when all done');
    assert.ok(result.includes('100%'), 'badge title should show 100%');
  });
  ...
  test('Non-loop run renders correctly with stories arg', () => {
    ...
    assert.ok(!result.includes('loop-badge'), 'no badge for non-loop run');
